...
func pullTrackingSearchFromCache(priority _types.Priority, keys []string) ([]*trackingSearch, error) {
	...
	for {
		...
		for _, key := range keys {
			if os, err := _cache.Get(key, "reqTime", "carrierCode", "language", "trackingNo", "clientAddr", "status", "crawlerResult", "crawlerName", "crawlerStartTime", "crawlerEndTime"); err != nil {
				...
				crawlerRspJson := strings.TrimSpace(_utils.AsString(os[6]))
				...
				trackingSearch := trackingSearch{
					...
					CrawlerName:      _utils.AsString(os[7]),
					CrawlerStartTime: _utils.AsTime(os[8]),
					CrawlerEndTime:   _utils.AsTime(os[9]),
					Events:           events,
					CrawlerCode:      trackingResult.Code,
					Err:              trackingResult.CMess,
					RawText:          crawlerRspJson,
				}
...
func saveTrackingResultToDb(trackingSearchList []*trackingSearch) {
	...
	for _, ts := range trackingSearchList {
		...
		if carrierPo != nil {
			...
			trackingId := _db.SaveTrackingToDb(carrierPo.Id, ts.Language, ts.TrackingNo, ts.DoneTime, ts.DonePlace, ts.Src, ts.CrawlerName, now, ts.Done)
...
func saveLogToDb(trackingSearchList []*trackingSearch) {
	...
	for _, ts := range trackingSearchList {
		if ts.CrawlerName == "" {
			// 没有匹配到爬虫。但是也应该记录日志。
			_db.SaveTrackingLogToDb(0, ts.TrackingNo, 0, 0, 0, ts.ClientAddr, 0, now, ts.Src, now, operator,
				ts.ReqTime, ts.CrawlerStartTime, ts.CrawlerEndTime, ts.RawText, fmt.Sprintf("没有匹配到爬虫(carrier-code=%s)", ts.CarrierCode))
		} else {
			carrierPo := _db.QueryCarrierByCode(ts.CarrierCode)
			...
			if carrierPo != nil {
				...
				resultNote = fmt.Sprintf("没有匹配到运输商(carrier-code=%s)", ts.CarrierCode)
			}
			_db.SaveTrackingLogToDb(carrierId, ts.TrackingNo, matchType, countryId, int(timing), ts.ClientAddr, resultStatus, now, ts.Src, now, operator,
				ts.ReqTime, ts.CrawlerStartTime, ts.CrawlerEndTime, ts.RawText, resultNote)
		}
	}
}
